...
void main() {
  ...
  vec3 backdrop = mix(vec3(0.094, 0.094, 0.106), vec3(1.0), step(0.5, uLightMode));
  ...
  if (uLightMode > 0.5) {
    ...
    float fibers = pow(smoothstep(0.06, 0.9, fiberField) * edgeFade, 1.2);
    float atmosphere = (center * 0.03 + cloud * 0.02) * edgeFade;
    vec3 fiberInk = mix(backdrop, uLineColor, 0.72);
    vec3 airColor = mix(backdrop, uGlowColor, 0.2);

    outputColor = mix(backdrop, airColor, atmosphere);
    outputColor = mix(outputColor, fiberInk, fibers * 0.5);
  } else {
